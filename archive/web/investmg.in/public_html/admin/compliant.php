...
// Build query
$query = "SELECT * FROM calls WHERE is_compliant = 1 AND date BETWEEN ? AND ?";
...
$compliantCalls = $db->fetchAll($query, $params);

// Get statistics by user
$userStats = $db->fetchAll(
    "SELECT user_id, COUNT(*) as count
     FROM calls
     WHERE is_compliant = 1 AND date BETWEEN ? AND ?
...
<head>
    ...
    <title>Compliant Calls - Call Recording Compliance</title>
    ...
    <div class="dashboard">
        ...
        <aside class="sidebar">
            ...
            <nav class="sidebar-nav">
                ...
                <a href="compliant.php" class="active">Compliant Calls</a>
        ...
        <main class="main-content">
            <div class="header">
                <h1>Compliant Calls</h1>
                <div class="header-actions">
                    <span>Total: <?php echo count($compliantCalls); ?> calls</span>
                </div>
            </div>
                    ...
                    <div style="padding: 20px;">
                        <?php foreach ($userStats as $stat): ?>
                            <div style="display: flex; justify-content: space-between; margin-bottom: 15px; align-items: center;">
                                ...
                                <span class="status-badge status-green"><?php echo $stat['count']; ?> calls</span>
                            </div>
                        <?php endforeach; ?>
                    </div>
                </div>
            ...
            <div class="table-container">
                <div class="table-header">
                    <h3>Compliant Call Details</h3>
                    <p style="color: #7f8c8d; font-size: 0.9rem; margin-top: 5px;">Calls with valid recordings</p>
                </div>
                <?php if (empty($compliantCalls)): ?>
                    <div class="empty-state">
                        <h3>No Compliant Calls</h3>
                        <p>No compliant calls found in the selected period.</p>
                    </div>
                <?php else: ?>
                    <table id="compliantTable" class="display report-table" style="width:100%">
                        <thead>
                            <tr>
                                <th>User ID</th>
                                <th>Call ID</th>
                        ...
                        <tbody>
                            <?php foreach ($compliantCalls as $call): ?>
                                <tr>
                                    ...
                                    <td>
                                        <span class="shortfall-positive">
                                            <?php echo htmlspecialchars($call['reason'] ?? 'Recording matched'); ?>
                                        </span>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
